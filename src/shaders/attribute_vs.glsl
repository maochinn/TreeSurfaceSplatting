...
layout(std140, column_major, binding = 1) uniform Raycast
{
...
layout(std140, binding = 3) uniform Parameter
{
    vec3 material_color;
...
void main()
{
    mat4 modelview = modelview_matrix * u_model;

    vec4 c_eye = modelview * vec4(c, 1.0);
    vec3 u_eye = radius_scale * mat3(modelview) * u;
    vec3 v_eye = radius_scale * mat3(modelview) * v;
...
#endif
    // gl_PointSize = max(w[0] * viewport.z, w[1] * viewport.w) + 1.0;
